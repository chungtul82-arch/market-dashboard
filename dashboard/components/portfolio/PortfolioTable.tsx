...
function InlinePicker<T extends string>({
  ...
  return (
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 left-0 bg-card border border-border rounded-xl shadow-xl min-w-[110px] max-h-52 overflow-y-auto">
          <button key="__clear__" onClick={() => handleSelect('' as T)}
            className="w-full text-left px-3 py-1.5 text-xs hover:bg-muted/60 text-muted-foreground/50 transition-colors">
            — 미지정
          </button>
          {options.map(o => (
      ...
      <div className="bg-card rounded-xl border border-border p-4 overflow-x-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-muted-foreground/60">배지 클릭 → 변경 · ✏️ → 전체 수정</p>
          </div>
        </div>
                ...
                <tr key={h.symbol} className={cn('border-t border-border hover:bg-muted/20 transition-colors', selected.has(h.symbol) && 'bg-[#6366f1]/5')}>
                  ...
                  <td className="py-2 px-3">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      {h.market && (
                        <span className="text-xs px-1.5 rounded border border-border text-muted-foreground/60">
                          {h.market}
                        </span>
                      )}
                    </div>
